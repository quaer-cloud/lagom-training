...
/**
  * This provides an event sourced behavior. It has a state, [[AccountserviceState]], which
  ...
  */
object AccountserviceBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[AccountserviceCommand]): Behavior[AccountserviceCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, AccountServiceEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[AccountserviceCommand, AccountServiceEvent, AccountserviceState](
        persistenceId = persistenceId,
        emptyState = AccountserviceState.initial,
...
/**
  ...
  */
case class AccountserviceState(message: String, timestamp: String) {
  def applyCommand(cmd: AccountserviceCommand): ReplyEffect[AccountServiceEvent, AccountserviceState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: AccountServiceEvent): AccountserviceState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[AccountServiceEvent, AccountserviceState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[AccountServiceEvent, AccountserviceState] =
    Effect
...
object AccountserviceState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: AccountserviceState = AccountserviceState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey: EntityTypeKey[AccountserviceCommand] = EntityTypeKey[AccountserviceCommand]("AccountServiceAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[AccountserviceState] = Json.format
}

/**
  * This interface defines all the events that the AccountServiceAggregate supports.
  */
sealed trait AccountServiceEvent extends AggregateEvent[AccountServiceEvent] {
  def aggregateTag: AggregateEventTag[AccountServiceEvent] = AccountServiceEvent.Tag
}

object AccountServiceEvent {
  val Tag: AggregateEventTag[AccountServiceEvent] = AggregateEventTag[AccountServiceEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends AccountServiceEvent
...
/**
  ...
  */
trait AccountserviceCommandSerializable

/**
  * This interface defines all the commands that the AccountServiceAggregate supports.
  */
sealed trait AccountserviceCommand
    extends AccountserviceCommandSerializable
...
/**
  ...
  * application loader.
  */
object AccountserviceSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[AccountserviceState],
