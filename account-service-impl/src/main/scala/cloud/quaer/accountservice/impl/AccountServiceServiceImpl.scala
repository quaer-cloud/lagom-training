...
import cloud.quaer.accountservice.api
...
/**
  * Implementation of the AccountserviceService.
  */
class AccountServiceServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends AccountserviceService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[AccountserviceCommand] =
    clusterSharding.entityRefFor(AccountserviceState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(AccountServiceEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[AccountServiceEvent]
